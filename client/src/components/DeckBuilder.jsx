  ...
  const API_BASE_DECKCODE = window.location.hostname === "localhost"
    ? "http://localhost:3001"
    : "https://deckcode-server-production.up.railway.app";

  const API_BASE_DECKLOG = window.location.hostname === "localhost"
    ? "http://localhost:3001"
  ...
  function DeckBuilder() {
    ...
    const allTags = [
      ...
      "嬰兒", "DEV_IS", "ReGLOSS", "Justice", "食物", "香菇", "夏季"
    ];
    ...
    const allCards = [...cardList, ...cardListBP02, ...cardListBP03, ...cardListBP04, ...cardListSD01, ...cardListSD02, ...cardListSD03, ...cardListSD04, 
                      ...cardListSD05, ...cardListSD06, ...cardListSD07, ...cardListSD08, ...cardListSD09, ...cardListPR, ...cardListBD24, ...energyCardList].map(card => ({
                      ...card,
    ...
    const handleExportCode = async () => {
      ...
      console.log("🟦 儲存資料 payload:", payload);
      const code = Array.from({ length: 6 }, () =>
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".charAt(Math.floor(Math.random() * 36))
      ).join("");

      try {
        await fetch(`${API_BASE_DECKCODE}/save/${code}`, {
        method: "POST",
    ...
    const handleImportCode = async () => {
      ...
      try {
        ...
        if (shareCode.length === 5 && !shareCode.includes("-")) {
          // 👇 是 decklog 的代碼
          const res = await fetch(`${API_BASE_DECKLOG}/import-decklog/${shareCode}`);
          if (!res.ok) throw new Error("decklog 載入失敗");
          data = await res.json();
        } else {
          // 👇 是自己分享儲存的代碼
          const res = await fetch(`${API_BASE_DECKCODE}/load/${shareCode}`);
    ...
    const handleExportImage = () => {
      ...
      const oshi = Object.values(grouped).filter(c => c.type === "Oshi");
      const deck = Object.values(grouped).filter(c => c.type !== "Oshi" && c.type !== "Energy");
      const energy = Object.values(grouped).filter(c => c.type === "Energy");
      ...
      const html = `<!DOCTYPE html><html><head><title>我的牌組</title>
        <style>
          body { font-family: sans-serif; padding: 20px; background: rgb(245, 250, 253); }
          h1 { margin-bottom: 16px; }
          h2 { margin: 20px 0 8px; font-size: 18px; border-bottom: 1px solid #ccc; }
          .grid { display: flex; flex-wrap: wrap; gap: 10px; }
          .card { position: relative; width: 180px; height: 250px; }
        ...
        <div id="deckArea">
          ...
          <h2>🌟 主推卡</h2>
          <div class="grid">
            ${oshi.map(createCardHTML).join("")}
          </div>

          <h2>📦 主卡組</h2>
          <div class="grid">
            ${deck.map(createCardHTML).join("")}
          </div>

          <h2>⚡ 能量卡</h2>
          <div class="grid">
            ${energy.map(createCardHTML).join("")}
          </div>
    ...
    const renderZoomCard = () => {
      ...
      return (
        ...
        >
          <div
            className="flex flex-col md:flex-row gap-4 p-4 bg-white rounded-xl max-w-[90vw] w-full justify-center items-center relative"
            onClick={(e) => e.stopPropagation()} // 避免點內部關閉 modal
          >
            <img
              src={zoomImageUrl}
              alt="原圖"
              className="w-full md:w-1/2 aspect-[630/880] object-contain shadow-lg rounded-xl" 
            />
            {translatedImageUrl && (
            <img
              src={translatedImageUrl}
              alt="翻譯圖"
              className="w-full md:w-1/2 aspect-[1120/1080] object-contain shadow-lg rounded-xl"
      ...
      <div className="flex flex-col h-screen max-h-screen bg-blue-50">
        <SearchBar
          ...
          <div className="flex flex-col md:flex-row flex-1 relative">
            ...
            {isMobile && (
              <button
                className="fixed bottom-4 right-4 z-50 md:hidden bg-blue-600 text-white rounded-full px-4 py-2 shadow-lg"
                onClick={() => setDeckVisible(prev => !prev)}
              >
                {deckVisible ? "🔽 收起牌組" : "📂 展示牌組"}
              </button>
            )}

            {/* ✅ Deck 區域（手機浮出，桌機右側） */}
            {deckVisible && isMobile && (
              <div
            ...
            {(deckVisible || !isMobile) && (
              <div
                ref={deckRef}
                className={`
                  bg-zinc-100 px-4 py-4 
                  border-t md:border-t-0 md:border-l
                  w-full md:w-[47%] 
                  ${deckVisible && window.innerWidth < 768
                    ? 'fixed top-14 bottom-0 left-0 right-0 z-40 max-h-[70vh] overflow-y-auto rounded-b-xl shadow-xl bg-zinc-100'
                    : 'z-10'}  // ⚠️ 加上 z-index 控制
                `}
              >
            ...
            >
              ...
              >
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  ...
                  <br />
                  6/17更新內容 : 第四彈卡表上線；修正部分錯誤
                  <br />
                  7/10更新內容 : 官方5碼代碼&本地6碼代碼BUG修正；輸出圖表排版&優化
                  <br />
                  7/12更新內容 : 介面優化，針對在手機不好操作的問題；增加部分pr卡和特殊卡圖
                  <br />
                  7/17更新內容 : 部分效果文修正
                </p>
